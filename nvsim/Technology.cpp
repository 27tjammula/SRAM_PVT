...
void Technology::ApplyCalibration(double _vddOverride, double _ionScale, double _ioffScale) {
	if (_vddOverride > 0)
		vdd = _vddOverride;

	if (_ionScale > 0) {
		for (int i = 0; i <= 100; i++) {
			currentOnNmos[i] *= _ionScale;
			currentOnPmos[i] *= _ionScale;
		}
	}

	if (_ioffScale > 0) {
		for (int i = 0; i <= 100; i++) {
			currentOffNmos[i] *= _ioffScale;
			currentOffPmos[i] *= _ioffScale;
		}
	}
