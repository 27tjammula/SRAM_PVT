...
void InputParameter::ReadInputParameterFromFile(const std::string & inputFile) {
	...
	while (fscanf(fp, "%[^\n]\n", line) != EOF) {
		...
		if (!strncmp("-VddOverride", line, strlen("-VddOverride"))) {
			sscanf(line, "-VddOverride (V): %lf", &vddOverride);
			continue;
		}
		if (!strncmp("-IonScale", line, strlen("-IonScale"))) {
			sscanf(line, "-IonScale: %lf", &ionScale);
			continue;
		}
		if (!strncmp("-IoffScale", line, strlen("-IoffScale"))) {
			sscanf(line, "-IoffScale: %lf", &ioffScale);
			continue;
		}
...
void InputParameter::PrintInputParameter() {
	...
	if (vddOverride > 0 || ionScale != 1 || ioffScale != 1) {
		cout << "Calibration: VDD override = ";
		if (vddOverride > 0)
			cout << vddOverride << "V";
		else
			cout << "none";
		cout << ", Ion scale = " << ionScale << ", Ioff scale = " << ioffScale << endl;
	}
}
